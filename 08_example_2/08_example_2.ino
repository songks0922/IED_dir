...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw <= 200 && dist_raw > dist_min){
    analogWrite(PIN_LED,510 - (2.55 * dist_raw)); //거리가 멀어질수록 row에 가까워짐   
  }else if(dist_raw >200 && dist_raw <= dist_max){
    analogWrite(PIN_LED,-510 + (2.55 * dist_raw));  //거리가 멀어질수록 high에 가까워짐
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  //           = 100,000 micro*sec * 0.001 m       illi/micro * 0.5 * 346 meter/sec
